...
const Contact = ({ personalInfo }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setFormStatus({ 
      status: 'success', 
      message: 'Your email client has been opened to send the message. Thank you!' 
    });
  };
         ...
         <div className="min-h-[400px] flex flex-col justify-center">
            {formStatus.status === 'idle' || formStatus.status === 'submitting' ? (
               ...
               <div className="text-center bg-green-500/10 border border-green-500 text-green-300 p-8 rounded-lg flex flex-col items-center justify-center h-full">
                  <h3 className="text-2xl font-bold mb-4 text-white">Check Your Email Client</h3>
      ...
      <>
        ...
        <div className="min-h-screen bg-bg-primary text-white flex">
            ...
            <main className="flex-grow p-8 overflow-y-auto">
              ...
              {activeTab === 'articles' && (
                  <div>
                      <h2 className="text-3xl font-bold mb-6 font-heading">Edit Articles</h2>
                       <div className="space-y-6">
                          {articlesData.map((item, index) => (
                              <div key={item.id || index} className="bg-bg-secondary p-6 rounded-lg border border-border-color relative">
                                  ...
                                  <div className="mb-4">
                                      ...
                                      <input type="text" value={item.title} onChange={e => handleItemChange(setArticlesData, index, 'title', e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 bg-slate-700 border-slate-600 text-white leading-tight focus:outline-none focus:shadow-outline focus:border-accent-primary" />
                                  </div>
                                  {renderInput(`Category`, item.category, e => handleItemChange(setArticlesData, index, 'category', e.target.value))}
                                  <ImageInput label="Article Image" imageUrl={item.imageUrl} onUrlChange={e => handleItemChange(setArticlesData, index, 'imageUrl', e.target.value)} onFileChange={async e => handleItemChange(setArticlesData, index, 'imageUrl', await fileToBase64(e.target.files[0]))} onAiEdit={() => openAiModal(item.imageUrl, (newImg) => handleItemChange(setArticlesData, index, 'imageUrl', newImg))} showImageToggle={true} isImageShown={!!item.showImage} onImageToggleChange={e => handleItemChange(setArticlesData, index, 'showImage', e.target.checked)} />
                                  {renderInput(`Link`, item.link, e => handleItemChange(setArticlesData, index, 'link', e.target.value))}
                                  {renderTextarea(`Description`, item.description, e => handleItemChange(setArticlesData, index, 'description', e.target.value), { fieldKey: `article_desc_${index}`, prompt: "Rewrite this article description to be more engaging.", currentValue: item.description, onGenerated: (newText) => handleItemChange(setArticlesData, index, 'description', newText) })}
                              </div>
                          ))}
                      </div>
                  ...
                  <div>
                      ...
                      <div className="bg-bg-secondary p-6 rounded-lg border border-border-color mb-6">
                       ...
                       <div className="bg-bg-secondary p-6 rounded-lg border border-border-color">
                          ...
                          <ToggleSwitch 
                              label="Enable On-Site AI Chatbot"
                              checked={!!aiFeaturesConfig.showChatbot}
                              onChange={e => setAiFeaturesConfig(prev => ({...prev, showChatbot: e.target.checked}))}
                          />
                          <p className="text-xs text-slate-400 mt-2 mb-4">Shows or hides the AI assistant chat bubble on the portfolio page. Requires a valid API key.</p>
                          <ToggleSwitch 
                          ...
                          />
                          <p className="text-xs text-slate-400 mt-2 mb-4">This toggles the 'Enhance' buttons for text fields in the admin panel. Requires a valid API key.</p>
                          <ToggleSwitch 
                          ...
                          />
                          <p className="text-xs text-slate-400 mt-2">This toggles the "Edit with AI" button for all images in the admin panel. Requires a valid API key.</p>
                      </div>
